...
class Drum:
    ...
    def __init__(self, pitch: int, velocity: int = 64, duration: int = 1, time: int = 0):
...
class DrumKit:
    ...
    def add_drum(self, drum_name: str, velocity: int = 64, duration: int = 1, time: int = 0) -> None:
        """
        ...
        """
        if drum_name not in self.GM1_DRUM_MAP:
            raise ValueError(f"Invalid drum name: {drum_name}. Please use a valid drum name from the GM1_DRUM_MAP.")
        
        drum = Drum(self.GM1_DRUM_MAP[drum_name], velocity, duration, time)
