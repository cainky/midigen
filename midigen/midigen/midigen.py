...
class MidiGen:
    ...
    def set_time_signature(self, numerator: int, denominator: int):
        if not (isinstance(numerator, int) and isinstance(denominator, int)):
            raise ValueError("Invalid time signature value: both numerator and denominator must be integers")

        if numerator <= 0 or denominator <= 0:
            raise ValueError("Invalid time signature value: both numerator and denominator must be positive integers")
        
        # Remove existing 'time_signature' messages
        self._track = [msg for msg in self._track if msg.type != 'time_signature']

        self.time_signature = MetaMessage('time_signature', numerator=numerator, denominator=denominator)
    ...
    def set_key_signature(self, key):
        ...
        if key not in valid_keys:
            raise ValueError("Invalid key. Please use a valid key from the list: {}".format(valid_keys))
